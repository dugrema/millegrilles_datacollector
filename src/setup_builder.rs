...
use crate::maintenance::maintenance_thread;
use crate::setup_mongodb::prepare_mongodb_index;

static DOMAIN_MANAGER: StaticCell<DataCollectorDomainManager> = StaticCell::new();
...
async fn initialiser<M>(middleware: &'static M) -> Result<(&'static DataCollectorDomainManager, FuturesUnordered<JoinHandle<()>>), CommonError>
where M: Middleware
{
    let gestionnaire = DataCollectorDomainManager::new();
    ...
    futures.push(spawn(maintenance_thread(gestionnaire, middleware)));

    // Preparer des ressources additionnelles
    prepare_mongodb_index(middleware).await
        .expect("prepare_mongodb_index");

    Ok((gestionnaire, futures))
}

async fn thread_entretien<M>(_gestionnaire: &DataCollectorDomainManager, middleware: &M)
where M: Middleware
{
